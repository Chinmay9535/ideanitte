...
const GROQ_API_KEY = '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';
const GROQ_API_URL = 'https://api.groq.com/openai/v1/chat/completions';

// OpenWeatherMap API Configuration
const WEATHER_API_KEY = '83bba9576c5117e01c514eb966d541d4'; // Replace with your OpenWeatherMap API key
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            marker.on('dragend', async function(event) {
                ...
                if (isNaN(position.lat) || isNaN(position.lng)) {
                    console.error('Invalid marker coordinates after drag:', position);
                    showNotification('Invalid marker position', 'error');
                    return;
                }
                ...
                navigator.geolocation.getCurrentPosition(
                    async function(position) {
                        ...
                        if (isNaN(lat) || isNaN(lng)) {
                            ...
                            showNotification('Could not get valid location coordinates', 'error');
                            return;
                        }
    ...
    currentLocationBtn.addEventListener('click', async function() {
        try {
            ...
            // Validate coordinates
            if (isNaN(lat) || isNaN(lng)) {
                console.error('Invalid coordinates from getCurrentPositionPrecise:', lat, lng);
                showNotification('Could not get valid location coordinates', 'error');
                    ...
                    document.querySelectorAll('.location-suggestion').forEach(suggestion => {
                        suggestion.addEventListener('click', function() {
                            ...
                            if (isNaN(lat) || isNaN(lon)) {
                                console.error('Invalid coordinates:', this.dataset.lat, this.dataset.lon);
...
async function updateSelectedLocation(lat, lng, accuracy = null) {
    try {
        ...
        if (isNaN(lat) || isNaN(lng)) {
            console.error('Invalid coordinates in updateSelectedLocation:', lat, lng);
            showNotification('Invalid location coordinates', 'error');
            return null;
        }
...
For each crop type, provide recommendations in the following structured format:
{
    "biofuel": {
        ...
        "description": "Detailed description of the processing of ${cropType} waste",
        "processingSteps": [
            "Step 1",
            "Step 2",
            "Step 3",
    ...
    "composting": {
        // Same structure as biofuel but for composting
    },
    "recycling": {
        // Same structure as biofuel but for recycling
    }
}
...
async function fetchCurrentAQI(lat, lng) {
    try {
        const response = await fetch(`${AIR_POLLUTION_API_URL}?lat=${lat}&lon=${lng}&appid=${WEATHER_API_KEY}`);
        const data = await response.json();
        
        if (data && data.list && data.list.length > 0) {
            // Extract just the AQI value from OpenWeatherMap API
            ...
            let standardAQI;
            switch(aqiValue) {
                case 1: standardAQI = 25; break;  // Good (0-50)
                case 2: standardAQI = 75; break;  // Fair (51-100)
                case 3: standardAQI = 125; break; // Moderate (101-150)
                case 4: standardAQI = 175; break; // Poor (151-200)
                case 5: standardAQI = 300; break; // Very Poor (201+)
                default: standardAQI = 50; break;
            }
...
function updateAQIDisplay(aqiData) {
    ...
    // We're using the standardized scale we converted from OpenWeatherMap's 1-5 scale
    if (value <= 50) {
    ...
    aqiElement.innerHTML = `
        ...
        <div class="pollutants-grid">
            ${Object.entries(aqiData.pollutants)
                ...
                .map(([key, value]) => {
                    // Format the pollutant name and add units
                    let name = key.toUpperCase();
                    if (key === 'pm25') name = 'PM2.5';
                    
                    // Add appropriate units for each pollutant
                    let unit = 'μg/m³';
                    if (key === 'co') unit = 'mg/m³';
...
function updateImpactMetrics(recommendations) {
    ...
    if (currentAQI !== null) {
        // Calculate improvement based on our standardized AQI scale
        const projectedAQI = Math.max(25, currentAQI.aqi * (1 - totalImpact.aqi / 100));
...
async function fetchHistoricalAirPollution(lat, lng, startTime, endTime) {
    try {
        // Convert timestamps to Unix timestamps if they aren't already
        const start = typeof startTime === 'number' ? startTime : Math.floor(new Date(startTime).getTime() / 1000);
        const end = typeof endTime === 'number' ? endTime : Math.floor(new Date(endTime).getTime() / 1000);
        
        const response = await fetch(
            `https://api.openweathermap.org/data/2.5/air_pollution/history?lat=${lat}&lon=${lng}&start=${start}&end=${end}&appid=${WEATHER_API_KEY}`
        );
        ...
        if (data && data.list && data.list.length > 0) {
            ...
            return data.list.map(item => {
                // Convert OpenWeatherMap AQI (1-5 scale) to standard AQI scale
                let standardAQI;
            ...
            throw new Error('Invalid historical air pollution API response');
        }
    } catch (error) {
